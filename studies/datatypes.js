 ...
 * Simple datatypes values are stored directly in a variable because they have a fixed sized memory no bigger than 8 bytes 
 * this is called BY COPY cause when reused the value is simply copied becuase it is small enough to do so!
 *
 * Complex datatypes include: array, objects, function
 * Complex datatypes are an indefinite size and cannot fit all its value into one 8 byte variable 
 * therefore most of its value has to BY REFERENCE, referencing outside of the variable it was declared in 
 *
 *
 */
...
// 5. OBJECT //
/*
 Objects store a collection of simple datatypes in the structure of key/value pairs
 */
...
// 6. FUNCTION//
/*
 Functions are a reusable piece of code that takes an argument to act differently
...
-Infinitey is displayed when a number exceeds the lower limit of the floating point numbers
 */
 
console.log(Infinity);                 // Infinity will result in Infinity
console.log(Number.POSITIVE_INFINITY); // Number.POSITIVE_INFINITY result in Infinity

console.log(-Infinity);                 // -Infinity will result in -Infinity
console.log(Number.NEGATIVE_INFINITY); // Number.NEGATIVE_INFINITY will result in -Infinity
